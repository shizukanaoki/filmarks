...
r9.Screen = {
    ...
    draw: function(str){
        ...
        var modalid = "modal" + i + "";
        var modal_obj = $("<div class='lyrics_modal' id='" + modalid + "' style='display: none;'>" + "<p class='comment'>" + convertCRLF(escapeHtml(content_str)) + "</p>" + "<p class='username'><span class='username_description'>投稿者：</span><a class='link' href='/users/" + user_id + "'>" + escapeHtml(username_str) + "</a></p>" + "<div class='lyrics_info'>"  + "<img src='" + escapeHtml(file_name_str) + "' class='album_img'>" + "<p class='artist_name'>" + escapeHtml(artist_name_str) + "</p>" + "<p class='album_title'><a class='link' href='/albums/" + album_id + "'>" + escapeHtml(album_title_str) + "</a></p>" + "<p class='song_title'><a class='link' href='/songs/" + song_id + "'>" + escapeHtml(song_title_str) + "</a></p>" + "</div>" + "</div>");
